...
void unpc_block( int16_t * pc1, int16_t * out, int32_t num, int16_t * coefs, int32_t numactive, uint32_t chanbits, uint32_t denshift )
{
	...
	if ( numactive == 0 )
	{
		...
		if ( (num > 1)  && (pc1 != out) )
			memcpy( &out[1], &pc1[1], (num - 1) * sizeof(int16_t) );
		return;
	}
	if ( numactive == 31 )
	{
